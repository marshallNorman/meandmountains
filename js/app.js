(() => {
  const APP = {
    actionState() {
      const triggerText = $(".nav-trigger").text();
    ...
    toggleNav() {
      const $dropdown = $(".nav-section_dropdown");
      $dropdown.toggleClass('open');
    },

    setNavButtonState() {
      const $nav = $(".js-nav-trigger-wrapper");
      let action = this.actionState();
      let navTriggerMarkup = `<button class="nav-trigger">${action}</button>`;
    ...
    parallaxBg() {
      ...
      const $body = $("body");
      const $footer = $(".footer");
      const footerMarginTop = parseInt($footer.css("marginTop"), 10);
    ...
    marketoForm() {
      $(".js-gated-link").on("click", function (e) {
        e.preventDefault();
        const id = $(this).attr("data-marketoid");
        MktoForms2.loadForm("//app-ab13.marketo.com", "238-UZQ-013", id, (form) => {
